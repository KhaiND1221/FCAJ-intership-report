...
**Mục đích**: Phân tích ảnh món ăn hoặc tên món và trả về JSON dinh dưỡng có cấu trúc. Đây là prompt được dùng nhiều nhất — mọi thao tác chụp ảnh và tìm kiếm thất bại trong DB đều đi qua prompt này.
...
Mỗi prompt đều bắt đầu bằng `You are Ollie` để neo model vào một persona nhất quán trên cả 10 loại action. Nếu không có neo này, model được fine-tune trên corpus đa dạng có thể chuyển giọng giữa chừng (trang trọng → thông thường, hoặc tiếng Anh → tiếng Việt không có trigger). Framing nhất quán cũng giúp việc hiệu chỉnh tone dễ đoán — nếu Ollie quá trang trọng ở một action, cách sửa là như nhau trên tất cả prompt.
...
Một lần gọi `analyzeFoodImage` điển hình:

- System prompt: ~400 token.
- Ảnh (base64 JPEG 1280px @ quality 85): ~800–2000 token tùy độ phức tạp hình ảnh.
- User message: ~30 token.
- **Tổng input**: ~1200–2400 token.
- **Output** (nutrition JSON): ~300–500 token.
- **Tổng cộng**: ~1500–3000 token.

Với giá Bedrock Qwen3-VL (~$0.002/1K input token, ~$0.006/1K output token, ap-southeast-2 2025), mỗi lần phân tích ảnh thức ăn tốn khoảng $0.003–$0.005. Với người dùng log 3 bữa/ngày qua camera, đó là ~$0.40/tháng chi phí Bedrock.
