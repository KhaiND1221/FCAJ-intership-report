# 4.5.2 AIEngine — Lambda Orchestrator 9 Action

`ai-engine` là một Lambda duy nhất xử lý mọi tương tác AI dạng text và giọng nói trong NutriTrack. Thay vì một Lambda cho mỗi tính năng (nhân bội cold start và IAM policy), ta gom tất cả vào một orchestrator với switch `action` ở tầng trên. AppSync custom query chuyển tiếp chuỗi `action` và JSON `payload`; handler điều phối đến nhánh tương ứng.
...
> **Phân tích ảnh đã chuyển**: chụp ảnh thực phẩm, đọc nhãn dinh dưỡng và quét mã vạch không còn xử lý ở đây. Chúng route qua Lambda `scan-image` chuyên dụng, proxy đến ECS FastAPI. Xem [4.5.5 ScanImage](/workshop/4.5.5-ScanImage).
...
1. **`GEN_FOOD_SYSTEM_PROMPT`** — dùng bởi `generateFoodNutrition`. Ép ra schema JSON cứng và quy tắc nhất quán calorie `Protein*4 + Carbs*4 + Fat*9 ≈ calories`.
...
Frontend gọi (ví dụ với voiceToFood):

```typescript
const res = await client.queries.aiEngine({
  action: 'voiceToFood',
  payload: JSON.stringify({ s3Key: 'voice/user-abc/rec-1.m4a' }),
});
...
- [4.5.5 ScanImage — proxy xử lý ảnh](/workshop/4.5.5-ScanImage)
