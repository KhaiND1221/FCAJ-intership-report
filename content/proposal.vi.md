...
| **Amazon S3** | Lưu trữ media người dùng theo 3 prefix có tổ chức: `incoming/` (ảnh gốc, tự xóa sau 1 ngày), `voice/` (ghi âm), `media/` (ảnh đã xử lý/resize). |
...
- **Tầng Xử lý:** 4 hàm Lambda xử lý toàn bộ logic:
  ...
  - `resizeImage` — S3 event trigger trên prefix `incoming/`, dùng thư viện `sharp` resize về 1024px và chuyển WebP chất lượng 80%
...
1. **Xây dựng Lý thuyết và Thiết kế Kiến trúc (Tháng 0):** Nghiên cứu cấu hình React Native/Expo với AWS Amplify Gen 2 và thiết kế kiến trúc serverless bao gồm tích hợp AI Bedrock. Phác thảo sơ đồ kiến trúc giải pháp và định nghĩa mô hình dữ liệu.
2. **Tính toán Chi phí và Kiểm tra Tính khả thi (Tháng 1):** Sử dụng AWS Pricing Calculator để ước tính chi phí (gọi Lambda, token Bedrock, dung lượng DynamoDB) cho 1.000 người dùng. Xác nhận tính khả thi kiến trúc với ràng buộc chi phí dưới $65/tháng.
3. **Phát triển, Kiểm thử, và Triển khai (Tháng 2-3):** Lập trình giao diện React Native với 6 tab chính, triển khai 4 Lambda handler bằng TypeScript, cấu hình schema GraphQL AppSync với 8 model, tích hợp luồng xác thực Cognito, và xử lý các trường hợp biên (JWT federation, DynamoDB table discovery). Kiểm thử với Amplify Sandbox và triển khai lên production.
4. **Tinh chỉnh và Tối ưu hóa (Sau Phát hành):** Đánh giá UX/UI dựa trên phản hồi người dùng, cải thiện tính năng gamification (video tiến hóa thú cưng, cơ chế thử thách), tối ưu prompt engineering AI cho độ chính xác, và sửa lỗi tích hợp (vd: lỗi `NoValidAuthTokens` trong Bedrock calls).
...
- **Backend:** AWS Amplify Gen 2 (TypeScript CDK), Lambda (Node.js 18, ARM64), AppSync (GraphQL), DynamoDB (8 bảng với GSI), Cognito (User Pools + Identity Pools), S3 (với lifecycle rules).
...
|-----------|------------|----------|
| **Giai đoạn Tiền kỳ** | 1 Tháng (Tháng 0) | Lập kế hoạch UI/UX trên Figma, rà soát tài liệu phiên bản cũ (chuyển từ Flutter sang React Native), thiết kế kiến trúc trên draw.io, và đánh giá các dịch vụ AWS. |
| **Triển khai** | 3 Tháng | **Tháng 1:** Khởi tạo môi trường AWS Amplify Gen 2, cấu hình Cognito auth + Google OAuth, định nghĩa 8 model DynamoDB, và xây dựng các component UI React Native cốt lõi (Home, Add Food, Kitchen tabs).<br>**Tháng 2:** Kết nối API AppSync GraphQL, triển khai Lambda `process-nutrition` và `friend-request`, xây dựng giao diện Tủ lạnh/Nhà bếp, và tích hợp pipeline upload ảnh S3 với trigger `resize-image`.<br>**Tháng 3:** Triển khai Lambda `ai-engine` Bedrock với 10 tác vụ AI, xây dựng hệ thống gamification (streaks, thú cưng tiến hóa, thử thách), thực hiện E2E testing trên 3 môi trường, sửa lỗi JWT auth, và ra mắt production. |
| **Sau Phát hành** | Liên tục | Tối ưu hiệu suất, tích hợp phản hồi người dùng, thiết lập pipeline build iOS, và mở rộng quy mô người dùng trong 1 năm. |
...
| **API Gateway** | $0.34/tháng | 270K requests với giá $1.25/1M |
...
| **Mơ hồ Table Discovery DynamoDB** | Trung bình | Thấp | Truyền tên bảng chính xác qua environment variables trong `backend.ts` (đã triển khai), tránh dùng `discoverTables()` |
