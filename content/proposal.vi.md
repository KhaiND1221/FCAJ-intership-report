...
- **AWS Lambda** (5 hàm chuyên biệt) xử lý toàn bộ logic backend — từ điều phối AI đến quản lý hệ thống bạn bè, xử lý hình ảnh, và định tuyến ảnh sang ECS.
...
![Kiến trúc Giải pháp NutriTrack](/FCAJ-intership-report/solution-architect/nutritrack-final.drawio.png)
...
| **AWS Lambda** | Xử lý logic nghiệp vụ qua 5 hàm chuyên biệt: `ai-engine` (9 tác vụ AI), `process-nutrition` (tra cứu thực phẩm lai ghép), `friend-request` (tính năng xã hội), `resize-image` (tối ưu ảnh từ S3 trigger), và `scan-image` (proxy ảnh/mã vạch/nhãn dinh dưỡng đến ECS FastAPI). |
...
| **Amazon ECR** | Kho container (không dùng trong production — NutriTrack host image FastAPI trên Docker Hub free tier để tránh chi phí ECR). |
...
- **Tầng Xử lý:** 5 hàm Lambda xử lý toàn bộ logic:
  - `aiEngine` — Điều phối 9 tác vụ AI (voiceToFood, generateRecipe, generateCoachResponse, generateFoodNutrition, fixFood, ollieCoachTip, calculateMacros, challengeSummary, weeklyInsight)
  ...
  - `scanImage` — Tải ảnh từ S3 và gửi đến ECS FastAPI service để phân tích ảnh đồ ăn, quét mã vạch và nhãn dinh dưỡng; xác thực qua JWT ngắn hạn ký bằng khóa từ Secrets Manager
...
4. **Giai đoạn 3 — Tích hợp, Kiểm thử & Ra mắt (Tháng 3):** Tích hợp đầy đủ Bedrock (Qwen3-VL) với 9 tác vụ AI, triển khai Lambda `scanImage` + ECS Fargate (FastAPI + ECS FastAPI), kiểm thử E2E trên 3 môi trường (Sandbox / `feat/phase3` / `main`), xử lý các lỗi biên (JWT federation, mơ hồ `discoverTables()`, `NoValidAuthTokens`), và ra mắt production.
...
- **Tích hợp AI:** Amazon Bedrock API (Qwen3-VL 235B, region ap-southeast-2), AWS Transcribe (async transcription jobs), embedded prompt engineering với JSON schema enforcement. 9 tác vụ AI được điều phối qua Lambda `aiEngine`; phân tích ảnh/mã vạch/nhãn được xử lý bởi Lambda `scanImage` proxy sang ECS FastAPI (ECS FastAPI).
- **Hạ tầng:** Docker/ECS Fargate cho backend FastAPI, VPC với public/private subnets, ALB, Auto Scaling, NAT Instance (2×t4g.nano) thay NAT Gateway, Docker Hub cho container images.
...
| **Giai đoạn 3 (Tháng 3)** | 1 tháng | Triển khai Lambda `aiEngine` với 9 tác vụ Bedrock (Qwen3-VL), triển khai Lambda `scanImage` + ECS Fargate (FastAPI + ECS FastAPI), xây dựng hệ thống gamification (streaks, thú cưng tiến hóa, thử thách), thực hiện E2E testing trên 3 môi trường (Sandbox / `feat/phase3` / `main`), sửa lỗi JWT + `discoverTables()`, và ra mắt production. |
...
| **Amazon Cognito** | $0/tháng | 1.000 MAU trên gói Essentials với giá $0.015/MAU — 10.000 MAU đầu miễn phí; 1.000 người dùng nằm trong free tier |
...
| **Amazon Bedrock (AI)** | **$46.80/tháng** | 90K lệnh gọi AI: input tokens ($17.20) + output tokens ($29.59) — Giá Standard Qwen3-VL (Sydney ap-southeast-2, +3% so với US East) |
| **ECS Fargate + ALB + NAT** | **$44/tháng** | 2 Fargate task (0.5 vCPU/1 GB, ~$17) + ALB (~$16) + 2×t4g.nano NAT Instance (~$9) + CloudWatch Logs (~$2); Docker Hub thay ECR (không tốn chi phí registry) |
| **Tổng Ước lượng** | **~$100/tháng** | **~$1.200/12 tháng** |
...
*Giá Sydney xác nhận qua AWS Pricing API (hiệu lực từ 2026-03-01).*
...
- **Gói đăng ký Premium:** xác thực mô hình giá (mục tiêu: tỉ lệ chuyển đổi một chữ số đủ bù baseline ~$100/tháng), phát hành phân tích ảnh không giới hạn, báo cáo AI coach tuần, và biểu đồ macro nâng cao.
...
Xây dựng trong chương trình thực tập **First Cloud AI Journey (FCAJ)** tại **Amazon Web Services Vietnam**. Nhóm được định danh là **Nhóm 11** trong chương trình AWS và hoạt động dưới tên nội bộ **NeuraX**.

| Thành viên | Vai trò | Trách nhiệm |
| --- | --- | --- |
| Lê Nguyễn Gia Hưng | AI/ML Lead (AI-1) | Tích hợp Amazon Bedrock, prompt engineering (AI coach Ollie), Lambda `aiEngine` (9 hành động AI), hybrid lookup `processNutrition` |
| Ngô Hoài Nam | AI/ML Engineer (AI-2) | Pipeline AWS Transcribe (`vi-VN`), Lambda `resizeImage`, tinh chỉnh VLM prompt |
| Trần Đặng Thiên Phước | Security Lead (IA-1) | IAM roles theo least-privilege, Cognito User Pool + Google OAuth2, AWS WAF, bảo mật API |
| Nguyễn Đình Khải | Security Engineer (IA-2) | Kiểm thử xâm nhập, CloudWatch alarms, CloudTrail audit, hạ tầng ECS, S3 |
| Chu Anh Quân | Frontend & Backend Lead (DEV) | Ứng dụng React Native / Expo, backend Amplify Gen 2, Lambda `friendRequest`, schema DynamoDB, AppSync resolvers |
